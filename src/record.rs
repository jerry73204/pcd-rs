...
impl PCDRecord for u8 {
    ...
    fn write_line<R: Write>(&self, writer: &mut R, _field_defs: &[FieldDef]) -> Fallible<()> {
        writeln!(writer, "{}", self)?;
        Ok(())
    }
}
